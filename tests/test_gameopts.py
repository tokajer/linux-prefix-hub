...
def test_a_custom_environment_is_built_without_touching_steam(tmp_path,
                                                              monkeypatch):
    """It belongs to no game, so nothing gets pointed at it."""
    from linux_prefix_hub.adapters import steam
    from linux_prefix_hub.core import gameopts
    root, tools = _fake_steam(tmp_path, monkeypatch)
    cfg = write(root / "config/config.vdf",
    ...
    result = gameopts.turn_on(gameopts.as_game("daoc"))

    assert result.ok, result.message
    copy = tools / "LinuxPrefixHub-daoc"
    assert (copy / "proton").exists()
    assert gameopts.built_from(copy) == "GE-Proton10-34"
    # Steam's settings are untouched, byte for byte -- and no .bak, because
    # nothing was written. The user picks this one themselves.
    assert cfg.read_text() == before
    assert not cfg.with_suffix(".vdf.bak").exists()
    assert steam.compat_tool("daoc") == ""


def test_a_custom_name_survives_being_a_directory(tmp_path, monkeypatch):
    """The slug is the id; the typed name is kept so it can be shown back."""
    from linux_prefix_hub.core import gameopts
    _root, tools = _fake_steam(tmp_path, monkeypatch)

    gameopts.turn_on(gameopts.as_game("Dark Age of Camelot"))

    assert (tools / "LinuxPrefixHub-Dark-Age-of-Camelot").is_dir()
    listed = gameopts.custom_environments()
    assert [g["game_name"] for g in listed] == ["Dark Age of Camelot"]
    assert listed[0]["app_id"] == "Dark-Age-of-Camelot"


def test_a_custom_environment_gets_an_app_id_named_for_it(tmp_path,
                                                          monkeypatch):
    """Its id is never a number, which is the case the fallback exists for."""
    from linux_prefix_hub.core import gameopts
    _root, tools = _fake_steam(tmp_path, monkeypatch)

    gameopts.turn_on(gameopts.as_game("daoc"))
    text = (tools / "LinuxPrefixHub-daoc" / "user_settings.py").read_text()

    assert '"SteamAppId": "1"' in text


def test_turning_a_custom_environment_off_removes_only_its_copy(tmp_path,
                                                                monkeypatch):
    from linux_prefix_hub.core import gameopts
    _root, tools = _fake_steam(tmp_path, monkeypatch)
    gameopts.turn_on(gameopts.as_game("daoc"))

    result = gameopts.turn_off(gameopts.as_game("daoc"))

    assert result.ok, result.message
    assert not (tools / "LinuxPrefixHub-daoc").exists()
    assert (tools / "GE-Proton10-34").is_dir()
    assert gameopts.read("custom", "daoc")["enabled"] is False


def test_rebuild_all_covers_custom_environments_too(tmp_path, monkeypatch):
    from linux_prefix_hub.core import gameopts
    root, tools = _fake_steam(tmp_path, monkeypatch)
    gameopts.turn_on(_game())
    gameopts.turn_on(gameopts.as_game("daoc"))
...
# --- taking over what the script set up ----------------------------------
def _legacy_profile(home, name, env, base):
    folder = home / ".config" / gameopts_dir()
    write(folder / f"{name}.env", env)
    if base is not None:
        write(folder / f"{name}.base", base + "\n")
    return folder


def gameopts_dir():
    from linux_prefix_hub.core import gameopts
    return gameopts.LEGACY_PROFILE_DIR


def test_a_script_profile_can_be_taken_over(isolated_home, tmp_path,
                                            monkeypatch):
    """Your own daoc profile, as it really sits on disk."""
    from linux_prefix_hub.core import gameopts
    _root, tools = _fake_steam(tmp_path, monkeypatch)
    _legacy_profile(isolated_home, "daoc",
                    "# a comment\nSteamAppId=1\nMANGOHUD=1\n",
                    # the script really does store an absolute path here
                    "/home/you/.local/share/lutris/runners/wine/GE-Proton10-34")

    waiting = gameopts.importable()
    assert [e["name"] for e in waiting] == ["daoc"]
    # An absolute path is reduced to a build we actually have.
    assert waiting[0]["base"] == "GE-Proton10-34"

    assert gameopts.import_legacy(waiting[0]).ok
    profile = gameopts.read("custom", "daoc")
    assert profile["title"] == "daoc"
    assert profile["enabled"] is False        # taken over, not turned on
    assert gameopts.parse_custom(profile["custom"])["MANGOHUD"] == "1"
    # Offered once: it is ours now.
    assert gameopts.importable() == []


def test_an_import_leaves_the_scripts_own_instance_alone(isolated_home,
                                                         tmp_path,
                                                         monkeypatch):
    """Two tools writing into one directory is how a working setup is lost."""
    from linux_prefix_hub.core import gameopts
    _root, tools = _fake_steam(tmp_path, monkeypatch)
    theirs = tools / "Proton-Instance-daoc"
    theirs.mkdir()
    write(theirs / "proton", "#!/usr/bin/env python3\n")
    write(theirs / "user_settings.py", "user_settings = {'THEIRS': '1'}\n")
    _legacy_profile(isolated_home, "daoc", "MANGOHUD=1\n", None)

    gameopts.import_legacy(gameopts.importable()[0])

    assert (theirs / "user_settings.py").read_text() == \
        "user_settings = {'THEIRS': '1'}\n"


def test_an_unresolvable_base_falls_back_rather_than_carrying_over(
        isolated_home, tmp_path, monkeypatch):
    from linux_prefix_hub.core import gameopts
    _root, _tools = _fake_steam(tmp_path, monkeypatch)
    _legacy_profile(isolated_home, "old", "FOO=bar\n", "GE-Proton-Gone-9-9")

    assert gameopts.importable()[0]["base"] == gameopts.DEFAULT_FAMILY


# --- names people can read, and staleness a name cannot show -------------
...
def test_a_title_can_change_while_the_short_name_stays(tmp_path,
                                                       monkeypatch):
    """The folder is in somebody else's config; only our label moves."""
    from linux_prefix_hub.core import gameopts
    _root, tools = _fake_steam(tmp_path, monkeypatch)
    assert gameopts.turn_on(gameopts.as_game("daoc")).ok
    copy = tools / "LinuxPrefixHub-daoc"
    assert copy.is_dir()

    assert gameopts.rename("custom", "daoc", "Dark Age of Camelot").ok

    listed = gameopts.custom_environments()
    assert [g["game_name"] for g in listed] == ["Dark Age of Camelot"]
    assert listed[0]["alias"] == "daoc"
    # Nothing on disk moved, and a rebuild keeps the same folder.
    assert copy.is_dir()
    assert gameopts.turn_on(listed[0]).ok
    assert copy.is_dir()
    assert not (tools / "LinuxPrefixHub-Dark-Age-of-Camelot").exists()
    assert gameopts.read("custom", "daoc")["title"] == "Dark Age of Camelot"


def test_a_rename_to_nothing_is_refused():
    from linux_prefix_hub.core import gameopts
    gameopts.write("custom", "daoc", {"title": "daoc"})
    assert gameopts.rename("custom", "daoc", "   ").ok is False
    assert gameopts.read("custom", "daoc")["title"] == "daoc"
